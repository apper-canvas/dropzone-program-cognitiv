...
const FileCard = ({ 
  ...
  return (
    <div className={cn(
      ...
      file.status === "success" && "file-card success",
      file.status === "error" && "file-card error"
    )}>
      <div className="flex items-start space-x-4">
        ...
        <div className="flex-1 min-w-0 space-y-2">
          ...
          {/* Progress Bar */}
          {(file.status === "uploading" || (file.status === "success" && file.progress === 100)) && (
            <ProgressBar
          ...
          {file.status === "uploading" && (
            <div className="text-xs text-gray-500">
              {formatFileSize(file.uploadedBytes)} of {formatFileSize(file.size)} uploaded
            </div>
          )}
